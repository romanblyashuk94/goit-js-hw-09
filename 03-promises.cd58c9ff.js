...
function runPromisesGenerator(delay1, delayStep, promisesAmount) {
    for(let i = 1; i <= promisesAmount; i++){
        createPromise(i, delay1).then(({ position , delay  })=>_notiflixDefault.default.Notify.success(`Fulfilled promise ${position} in ${delay}ms`)
        ).catch(({ position , delay  })=>_notiflixDefault.default.Notify.failure(`Rejected promise ${position} in ${delay}ms`)
        );
        delay1 += delayStep;
    }
}
...
},{}]},["5rjme","8tzFL"], "8tzFL", "parcelRequire7bc7")
